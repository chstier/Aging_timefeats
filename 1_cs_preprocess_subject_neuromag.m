...
%%% of CamCAN-data (resting-state, previously maxfiltered) 

% written by Christina Stier, 2020/2022, modified in 2023
...
%% Do lcmv beamforming for each subject
for i = 1:length(proc_subjects)
   subject = all_subjects{i};
   subject = nmri_load_subject_most_advanced(subject);
   
   title_subj = fullfile([subject.id '_lcmv']);

   cfg=[];
   cfg.title=title_subj;
%  cfg.queue = 'fat';
   cfg.spm=0;
   cfg.fieldtrip=1;
   nmri_qsub(cfg,'cs_nmri_processing_lcmv',subject)
   
   pause('on')
   n = 210; % wait 3,5 Minutes to throw next job because of available matlab licences
   pause(n)
