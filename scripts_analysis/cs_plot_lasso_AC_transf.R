...
##### get PLS results
p = readMat('~/Lasso_50rep_allAC_av_transf.mat')
acc = as.numeric(p$av.acc)
mae = as.numeric(p$av.mae)
r2 = as.numeric(p$av.r2)
y_predicted = unlist(p$av.yhat)
betas_abs = array(unlist(p$reshaped.results), dim = c(40, 214)) 
betas = array(unlist(p$reshaped.notabs), dim = c(40, 214)) 
transfbetas_abs = array(unlist(p$feature.transf.abs), dim = c(40, 214)) 
transfbetas = array(unlist(p$feature.transf.notabs), dim = c(40, 214))
...
for (np in 1:n) {
  np_abs[np,1] = sum(transfbetas_abs[np,] != 0)
  np_notabs[np,1] = sum(transfbetas[np,] != 0)
}
...
g = g + ggtitle("AC 3-130 ms lags combined")

ggsave(file="Age_predicted_AC_combined_50rep.png", plot=g, dpi = 300, limitsize = TRUE, width = 8, height = 8)	
...
file = '~/sciebo/Features_age/labels_AC140.csv'
...
# combine
data_full = cbind(set, labels, time, tbetas_abs_av, tbetas_av, np_abs, np_notabs)
...
data_full %>%
  ...
  xlab('Autocorrelation time lags (ms)') 

ggsave('lasso_AC_absolute_transf.png', dpi = 300, width = 12, height = 8)
...
ggsave('lasso_AC_absolute_transf_notsorted.png', dpi = 300, width = 12, height = 8)
...
ggsave('lasso_AC_notabsolute_transf.png', dpi = 300, width = 12, height = 8)
...
ggsave('lasso_AC_notabsolute_transf_notsorted.png', dpi = 300, width = 12, height = 8)
